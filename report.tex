...
In addition, if we let $R$ represent the range of a particle, we can deduce its energy $E$ by using the regression model seen in \label{fig:air-energy} as follows:
\begin{align}
	E^2 = a(R_0 - R) + b = -aR
\end{align}
where $R_0$ is the range as computed before (x-intercept).
Solving for $R$ and taking $\ln$ on both sides gives
\begin{equation}
	\log R = 2\log(E) - \log(a).
\end{equation}
...
we obtain table \ref{tab:nist-comparison}.
...
The measured ranges are systematically lower than the NIST projected values, indicating that the effective stopping power in our setup was higher than in the pure reference gases.
